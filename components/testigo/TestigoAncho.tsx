...
function Banda({
  ...
  return (
    <span className="relative flex min-w-0 flex-1 items-center" style={{ height: alto }}>
      ...
      {dia.provisional && (
        <span
          className="ml-[3px] w-3 shrink-0 border-t-2 border-dashed border-t3"
          aria-hidden
        />
      )}
...
function FilaDensa({
  ...
  return (
    <div className="flex items-center gap-2 py-[1px]">
      ...
      <span className="w-6 shrink-0 text-right text-[10px] leading-none text-texto-apoyo">
  ...
  return (
    <>
      ...
      >
        ...
        <span
          className={`cifra w-14 shrink-0 text-right text-apoyo ${
      ...
      {/* El desglose se ABRE EN LA FILA en vez de flotar: en móvil no hay
          ...
          está mirando. */}
      {abierto && (
        <div className="mb-1.5 ml-[54px] border-l-2 border-borde pl-2.5 text-apoyo text-texto-apoyo">
          <span className="cifra">{dia.registros}</span> registros · T1{" "}
          <span className="cifra">{dia.registrosT1}</span> · T2{" "}
          <span className="cifra">{dia.registrosT2}</span> · T3{" "}
          <span className="cifra">{dia.registrosT3}</span>
          {dia.usuariosPago > 0 && (
            <>
              {" "}
              · <span className="cifra">{dia.usuariosPago}</span> de pago
            </>
          )}
          {dia.provisional && <span className="block text-vivo">Día abierto: aún puede cambiar.</span>}
        </div>
      )}
    </>
  );
