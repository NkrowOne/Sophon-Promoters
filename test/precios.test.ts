 ...
 *
 * Sophon publica el precio GLOBAL por usuario. De ahí sale un descuento fijo
 * que no llega al webmaster. Lo que se pinta es el resto, por cada 100
 * usuarios, que es la unidad en la que el programa de socios anuncia sus
 * tarifas.
 */

test("el precio del webmaster es el global menos diez céntimos, por cada 100", () => {
  // Los tres tiers de LV0 tal como los publica Sophon: 0,30 / 0,25 / 0,20 $ por
  // usuario. La tabla del programa los enseña como $20 / $15 / $10 por cada 100.
  assert.equal(formatearMicros(precioDelWebmaster("0.3")!), "20,00 $");
  assert.equal(formatearMicros(precioDelWebmaster("0.25")!), "15,00 $");
  assert.equal(formatearMicros(precioDelWebmaster("0.2")!), "10,00 $");

  // Y el de arriba del todo: T1 en LV6, 0,90 $ por usuario → $80 por cada 100.
  assert.equal(formatearMicros(precioDelWebmaster("0.9")!), "80,00 $");
});

test("el descuento reproduce el día real verificado contra la API", () => {
  /*
   * 2026-08-22, cuenta real, partnerLevel 0: 12 registros (3 T1 + 4 T2 + 5 T3)
   * sin compras. Sophon reportó 2,90 $ en total y 1,70 $ al webmaster. Si esta
   * aritmética no reproduce ese 1,70, el descuento de la tabla es mentira.
   */
  const delWebmaster =
    3n * precioDelWebmaster("0.3")! + 4n * precioDelWebmaster("0.25")! + 5n * precioDelWebmaster("0.2")!;
  assert.equal(delWebmaster, 170_000_000n * USUARIOS_POR_BLOQUE / 100n); // 1,70 $ × 100 usuarios / 100
});

test("el descuento es de diez céntimos por usuario, ni uno más", () => {
  ...
  // adaptarse sola.
  assert.equal(DESCUENTO_POR_USUARIO_MICROS, 100_000n);
  assert.equal(formatearMicros(DESCUENTO_POR_USUARIO_MICROS), "0,10 $");
...
test("un precio que no cabe en la realidad no se pinta: se descarta", () => {
   ...
   * por cada 100 (`$30`). El día que ese campo viniera ya multiplicado, el
   * descuento se quedaría en ruido y la pantalla anunciaría precios cien veces
  ...
  assert.equal(precioDelWebmaster("0.10"), null);
...
test("el reparto del Operador no está representado en ninguna parte", () => {
   ...
   * descuento entero y no dos sumandos. Si existiera la
